...
describe('Comments API', () => {
  ...
  describe('modificar comentarios', () => {
    ...
    it('modificar un comentario', async () => {
      ...
      expect(response.data.patched.text).toBe('<p>modificado!</p>')
  ...
  describe('listar comentarios', () => {
    it('listar comentarios para un post', async () => {
    ...
    it('listar todos comentarios ', async () => {
      const comment1 = makeFakeComment({ replyToId: null })
      const comment2 = makeFakeComment({
        postId: comment1.postId,
        replyToId: null
      ...
      const response = await axios.get('/all-comments',)
      return comments.map(commentsDb.remove)
    })
